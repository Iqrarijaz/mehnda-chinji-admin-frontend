...
const ColumnVisibilityDropdown = memo(function ColumnVisibilityDropdown({
    ...
    const visibilityDropdown = (
        <div className="bg-white dark:bg-slate-900 rounded p-3 border border-slate-200 dark:border-slate-800 min-w-[200px] shadow-none transition-colors duration-200">
            ...
            >
                {displayOptions.map((opt) => (
                    <div key={opt.value} className="hover:bg-slate-50 dark:hover:bg-slate-800/80 rounded py-1 px-1.5 transition-colors">
    ...
    return (
        <Dropdown dropdownRender={() => visibilityDropdown} trigger={["click"]} placement="bottomRight">
            <Button
                ...
                className={`rounded !border !border-slate-200 dark:!border-slate-700 !h-[40px] !px-3.5 !text-slate-700 dark:!text-slate-200 !bg-white dark:!bg-slate-800 hover:!bg-[#006666] hover:!text-white dark:hover:!bg-[#006666] dark:hover:!text-white flex items-center gap-1.5 !text-xs font-medium transition-all duration-200 shadow-none ${className}`}
            >
                Columns
            </Button>
        </Dropdown>
