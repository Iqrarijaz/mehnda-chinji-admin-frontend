...
const AddListingModal = React.memo(({ modal, setModal }) => {
    const formikRef = useRef(null);
    const queryClient = useQueryClient();
    const [fileList, setFileList] = useState([]);

    const initialValues = useMemo(() => ({
        title: "", description: "", price: "", place: "", sellerPhone: "",
        categoryEn: "", categoryUr: "", typeEn: "", typeUr: "", metadataString: "",
        negotiable: false, showPhoneNumber: true, isFeatured: false,
    }), []);
    ...
    const handleCreate = useMutation({
        ...
        onError: (error) => {
            toast.error(error?.errorMessage || error?.message || "Failed to create listing");
        },
    });
        ...
        <Modal
            title={
                <div className="flex items-center gap-2 px-0 py-1">
                    ...
                    <div>
                        <span className="text-lg font-bold text-[#006666] dark:text-teal-500 block mt-1 transition-colors duration-300">Add Marketplace Listing</span>
                    </div>
                </div>
            }
            centered
                        ...
                        <Form className="space-y-2">
                            <div className="modal-section">
                                ...
                                <FormField 
                                    ...
                                    name="description" 
                                    rows={4} 
                                    placeholder="Enter item condition, breed, age details..." 
                                    required 
                                />
                            </div>
                            ...
                            <div className="modal-section">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <SelectField label="Category (English)" name="categoryEn" options={[{label: 'Animals', value: 'Animals'}, {label: 'Vehicles', value: 'Vehicles'}, {label: 'Electronics', value: 'Electronics'}, {label: 'Property', value: 'Property'}, {label: 'Jobs', value: 'Jobs'}, {label: 'Services', value: 'Services'}]} required />
                                    <SelectField label="Category (Urdu)" name="categoryUr" options={[{label: 'جانور', value: 'جانور'}, {label: 'گاڑیاں', value: 'گاڑیاں'}, {label: 'الیکٹرانکس', value: 'الیکٹرانکس'}, {label: 'پراپرٹی', value: 'پراپرٹی'}, {label: 'نوکریاں', value: 'نوکریاں'}, {label: 'خدمات', value: 'خدمات'}]} required />
                                </div>
                                <div className="grid grid-cols-2 gap-4 mt-2">
                                    <SelectField label="Type (English)" name="typeEn" options={[{label: 'Cow', value: 'Cow'}, {label: 'Goat', value: 'Goat'}, {label: 'Car', value: 'Car'}, {label: 'Bike', value: 'Bike'}, {label: 'Mobile', value: 'Mobile'}, {label: 'House', value: 'House'}]} required />
                                    <SelectField label="Type (Urdu)" name="typeUr" options={[{label: 'گائے', value: 'گائے'}, {label: 'بکرا', value: 'بکرا'}, {label: 'گاڑی', value: 'گاڑی'}, {label: 'موٹر سائیکل', value: 'موٹر سائیکل'}, {label: 'موبائل', value: 'موبائل'}, {label: 'گھر', value: 'گھر'}]} required />
                                </div>
                            </div>
                            ...
                            <div className="modal-section">
                                ...
                                <div className="mt-3">
                                    <label className="text-slate-700 dark:text-slate-400 font-semibold text-xs mb-1 transition-colors block">Upload Images <span className="text-red-500">*</span></label>
                                    <Dragger {...uploadProps}>
                                        <p className="ant-upload-drag-icon"><InboxOutlined /></p>
                                        <p className="ant-upload-text">Click or drag images to upload</p>
                                        <p className="ant-upload-hint">Support for single or bulk upload. Max 5 images.</p>
                                    </Dragger>
                                </div>
                            ...
                            <div className="flex justify-end gap-2 pt-4 border-t border-slate-100 dark:border-slate-800 transition-colors">
                                <CustomButton label="Cancel" type="secondary" onClick={handleClose} disabled={isSubmitting || handleCreate.isPending} />
                                <CustomButton label="Create Listing" htmlType="submit" loading={isSubmitting || handleCreate.isPending} />
                            </div>
                        </Form>
                    )}
                </Formik>
