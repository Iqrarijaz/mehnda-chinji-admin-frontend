...
import { CREATE_EXPENSE } from "@/app/api/admin/water-supply";
...
const AddExpenseModal = React.memo(({ modal, setModal }) => {
    const formikRef = useRef(null);
    const queryClient = useQueryClient();
    const isModalOpen = modal?.state && modal?.name === "Add";

    // Since Formik doesn't handle AntD DatePicker well out of the box, we keep date in a separate state
    ...
    const createExpense = useMutation({
        ...
        onSuccess: (data) => {
            toast.success(data?.data?.message || "Expense logged successfully!");
            queryClient.invalidateQueries([ADMIN_KEYS.WATER_SUPPLY.EXPENSES_LIST]);
            handleCloseModal(true);
        },
        onError: (error) => {
            toast.error(error.errorMessage || "Failed to log expense.");
        },
    });
        ...
        <Modal
            title={
                <div className="flex items-center gap-2 px-0 py-1">
                    ...
                    <div>
                        <span className="text-lg font-bold text-[#006666] dark:text-teal-500 block mt-1 transition-colors duration-300">Log Operational Expense</span>
                    </div>
                </div>
            }
            centered
                        ...
                        <Form className="space-y-2">
                            ...
                            <div className="flex justify-end gap-2 pt-3 mt-3 border-t border-slate-100 dark:border-slate-800 transition-colors">
                                ...
                                <CustomButton
                                    label="Log Expense"
                                    htmlType="submit"
                                    loading={isSubmitting || createExpense.isLoading}
                                />
                            </div>
                        </Form>
                    )}
