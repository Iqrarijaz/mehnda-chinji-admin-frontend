...
import { useUpdateStoreProduct } from "../../hooks/useStore";
...
const UpdateProductModal = React.memo(({ modal, setModal, businessId, categories }) => {
    const formikRef = useRef(null);
    const queryClient = useQueryClient();

    const product = modal?.data;

    // Map images back to modal inputs
    const primaryImg = product?.images?.find(img => img.isPrimary)?.url || "";
    const remainingImgs = product?.images?.filter(img => !img.isPrimary) || [];
    const galleryImg1 = remainingImgs[0]?.url || "";
    const galleryImg2 = remainingImgs[1]?.url || "";
    ...
    const updateMutation = useUpdateStoreProduct(() => setModal({ name: null, state: false, data: null }));
        ...
        <Modal
            title={
                <div className="flex items-center gap-2 px-0 py-1">
                    ...
                    <div>
                        <span className="text-lg font-bold text-[#006666] block mt-1">Edit Product</span>
                    </div>
                </div>
            }
            centered
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        <SelectField
                                            ...
                                            options={categories.map((cat) => ({ value: cat._id, label: <span className="capitalize">{cat.name}</span> }))}
                                        />
                                    </div>
                                    <div>
                                        <SelectField
